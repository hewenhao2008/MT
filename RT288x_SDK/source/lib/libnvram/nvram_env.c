...
void nvram_init(int index)
{
	...
	for (i = 0; i < MAX_CACHE_ENTRY; i++) {
		if (NULL == (q = strchr(p, '='))) {
			LIBNV_ERROR("parsed failed - cannot find '='\n");
			break;
		}
		*q = '\0'; //strip '='
		...
		if (NULL == (q = strchr(p, '\0'))) {
			LIBNV_ERROR("parsed failed - cannot find '\\0'\n");
			break;
		}
...
{
	nvram_buftrav(index, NULL, 0);
}

int nvram_getall(int index, char *buff, int size)
{
	return nvram_buftrav(index, buff, size);
}
