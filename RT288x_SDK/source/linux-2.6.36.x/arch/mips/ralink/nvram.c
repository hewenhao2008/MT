...
#define NVRAM_MAJOR 251

static int ralink_nvram_major = NVRAM_MAJOR;
...
#ifdef  CONFIG_DEVTMPFS
	r = misc_register(&nvram_dev);
	if (r) {
		printk(KERN_ERR "nvram: can't misc_register on minor=%d, err:%d\n", 
			ralink_nvram_major, r);
		return r;
	}
	printk(KERN_DEBUG "nvram init misc device ok.\n");
#else
