...
static int br_query_fdb(const char *brname, struct fdb_entry **refdb, int *count)
{
#define CHUNK 128
...
static int br_cmd_showaddrs(int argc, char *const* argv)
{
	const char *brname = argv[1];
	struct fdb_entry *fdb = NULL;
	int i, count = 0;

	br_query_fdb(brname, &fdb, &count);
	if (!fdb) {
		return 0;
	}

	printf("port no\tmac addr\t\tl3addr\t\tageing timer\n");
	for (i = 0; i < count; i++) {
		...
		printf(NIPQUAD_FMT"\t\t", NIPQUAD(f->l3addr));
...
static const struct command commands[] = {
	...
	{ 1, "showaddrs", br_cmd_showaddrs,
	  "<bridge>\t\tshow a list of ip addrs"},
