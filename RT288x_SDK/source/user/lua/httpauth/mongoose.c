...
static process_id_t start_process(const char *interp, const char *cmd,
  ...
  char buf[500];
  process_id_t pid;
  fprintf(stderr, "TODO should not be here!\n");
  exit(-1);
  pid = fork();
  (void) env;
...
static void print_dir_entry(const struct dir_entry *de) {
  ...
  if (is_dir) {
    ...
    if (fsize < 1024) {
      ...
      mg_snprintf(size, sizeof(size), "%dk", (int) (fsize / 1024));
    } else if (fsize < 0x40000000) {
      mg_snprintf(size, sizeof(size), "%dM", (int) (fsize / 1048576));
    } else {
      mg_snprintf(size, sizeof(size), "%dG", (int) (fsize / 1073741824));
    }
  }
