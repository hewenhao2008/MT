 ...
 *
 * $Id: //WIFI_SOC/release/SDK_4_1_0_0/source/user/mtd_write/mtd.c#1 $
 *
...
mtd_write(int imagefd, int offset, int len, const char *mtd)
{
	...
	if (!quiet)
		fprintf(stderr, "start write ops...\n");
	...
	for (; len;) {
		...
		while (w > e) {
			...
			if (!quiet)
				fprintf(stderr, "[e:%08d, len:%08d, w:%02d%%]\n", e, len, (w*100/total_len));
			/* erase the chunk */
			if (ioctl (fd,MEMERASE,&mtdEraseInfo) < 0) {
				fprintf(stderr, "Erasing mtd failed: %s, %s\n", mtd, strerror(errno));
				free(buf);
				exit(1);
			}
			e += mtdInfo.erasesize;
...
void usage(void)
{
	...
	"        -c                      check fw version info before write.\n"
...
int main (int argc, char **argv)
{
	...
	while ((ch = getopt(argc, argv, "vwrcqe:o:l:")) != -1)
		switch (ch) {
			case 'r':
				boot = 1;
				break;
	...
	switch (cmd) {
		...
		case CMD_WRITE:
			if (quiet < 2)
				fprintf(stderr, "Writing from %s[%x:%x] to %s ...\n", imagefile, offset, len, device);

			if(fw_check) {
				char buff[512];
				if(image_check(imagefd, offset, len, buff)){
